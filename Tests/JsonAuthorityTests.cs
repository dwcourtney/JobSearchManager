...
internal static class JsonAuthorityTests
{
    ...
    internal static async Task RunAsync()
    {
        ...
        Require(snapshot.PipelineFingerprint != snapshot.CandidatePipelineFingerprint && snapshot.Matcher.AuthorityTag == "json-regex-v1", "Production identity must bind input/factual contracts");
    ...
    internal static Task ReportsAsync()
    {
        ...
        if(File.Exists(Path.Combine(root,"RegexEvaluation.cs")))
        {
            var metric=string.Concat(new[]{"RegexEvaluation.cs","AiHoldoutEvaluation.cs"}.Select(p=>File.ReadAllText(Path.Combine(root,p)).Replace("\r\n","\n",StringComparison.Ordinal)));
