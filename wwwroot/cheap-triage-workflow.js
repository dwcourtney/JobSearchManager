  ...
  function mount(panel,env=globalThis){
    ...
    function draw(){
      ...
      updateSummary=()=>{
        ...
        if(noUpdate){const matched=decision=>noUpdate.humanMatches.filter(x=>x.decision===decision).length;values=[
          ["Human review",`${r.reviewed} / ${r.cases.length}`],["KEEP decisions matched",`${matched("KEEP")} / ${r.counts.KEEP}`],["REJECT decisions matched",`${matched("REJECT")} / ${r.counts.REJECT}`],
          ["AMBIGUOUS",r.counts.AMBIGUOUS],["Running rules",noUpdate.rulesetVersion],["KEEP recall",percent(noUpdate.metrics.keepRecall)],
          ["Described KEEP recall",percent(noUpdate.metrics.describedKeepRecall)],["Title-only KEEP recall",percent(noUpdate.metrics.titleOnlyKeepRecall)],
          ["Changed decisions",noUpdate.changedDecisionCount],["Live mode",runtime?`${runtime.mode} · Non-gating`:"Loading…"]];}
      ...
      const action=make("section","");action.className="triage-current-action";action.setAttribute("aria-label","Current workflow action");card.append(action,metrics);
      const summary=()=>{
      ...
      if(reviewOpen){
        ...
        action.append(make("h4","No Rule Update Needed"),make("p",`Ruleset ${noUpdate.rulesetVersion} already matches all ${r.reviewed} reviewed decisions. No candidate was created and nothing needs to be released.`),make("p",`Validation: ${noUpdate.validationStatus}. Remaining provisional false rejects: ${noUpdate.metrics.falseRejectCount}.`));
      }else if(state.stage==="review-required"||state.stage==="review-progress"){
        action.append(make("p",r.reviewed?`${r.reviewed} of ${r.cases.length} reviewed; ${r.remaining} remaining.`:`${r.cases.length} potentially unsafe decisions need your review before rules can be updated.`),button(next,()=>{reviewOpen=true;draw();},true));
