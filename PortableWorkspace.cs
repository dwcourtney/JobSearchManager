...
internal sealed partial class PortableWorkspaceService
{
        ...
        JobHistoryDocument currentHistory)
    {
        ...
        if (document.Version is not (1 or 2 or 3 or 4 or 5 or 6 or 7 or CurrentVersion))
        {
    ...
    private void ValidateJobFit(JobFitConfiguration configuration)
    {
        ...
        if (configuration.PreferredWorkLocation is not null &&
            !WorkLocationPreference.IsSupported(configuration.PreferredWorkLocation))
        {
            throw new WorkspaceImportException(
                "Preferred work location must be an integer from 0 through 5.");
        }
