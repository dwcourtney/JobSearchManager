...
function evaluateJobFit(job) {
  if (job?.semanticClassificationStatus !== "complete") return null;
...
function createJobListItem(job) {
  ...
  if (jobFit) {
    appendJobBadge(
      ...
      `${JobFit.tooltip(jobFit)}\n\n✦ AI-generated by the default DeBERTa classifier.`);
...
function renderDetail(job) {
  ...
  renderJobFitDetail(evaluateJobFit(job), job);
  ...
  elements.qwenDeepAnalysisButton.textContent = job.qwenDeepAnalysis
    ? "Refresh Qwen Deep Analysis"
    : "Deep Analyze with Qwen";
  elements.qwenDeepAnalysisResult.hidden = !job.qwenDeepAnalysis;
  elements.qwenDeepAnalysisProvenance.textContent = job.qwenDeepAnalysis
    ? `Opt-in result from ${job.qwenDeepAnalysis.modelTag}; it does not affect the DeBERTa Job Fit score.`
    : "";
  elements.qwenDeepAnalysisText.textContent = job.qwenDeepAnalysis?.analysis || "";
...
async function deepAnalyzeSelectedJob() {
  ...
  elements.qwenDeepAnalysisButton.disabled = true;
  elements.qwenDeepAnalysisButton.textContent = "Qwen analysis running…";
  try {
    ...
    if (!response.ok) throw new Error("Qwen deep analysis is currently unavailable.");
    ...
    elements.qwenDeepAnalysisButton.textContent = "Deep Analyze with Qwen";
  }
}
