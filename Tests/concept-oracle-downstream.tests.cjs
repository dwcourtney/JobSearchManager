...
for(const signals of [[],[{conceptId:'technical.software-development',preference:'ideal'}],[{conceptId:'work.deployment',preference:'avoid'}]]) for(const pair of pairs) for(const preference of [null,0,3,5]) for(const tolerance of [null,0,5]) {
 state.jobFitSignals=signals;state.preferredWorkLocation=preference;state.travelTolerance=tolerance;
...
for(const [tolerance,score] of [[0,2],[5,5]]) {
 state.preferredWorkLocation=null;state.travelTolerance=tolerance;
 const id=catalog.concepts.find(c=>c.travelLevel===5).id;
 assert.equal(context.evaluateJobFit({detectedConcepts:[{conceptId:id}],semanticClassificationStatus:'complete'}).score,score);
}
delete state.preferredWorkLocation;delete state.travelTolerance;
assert.equal(context.evaluateJobFit({detectedConcepts:[],semanticClassificationStatus:'complete'}).score,5);
console.log('PASS missing preferences and controlled travel 2/10 versus 5/10');
